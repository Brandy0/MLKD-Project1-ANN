			  ...
			  int list_errors)
{
	...
	epochs = BPNN_EPOCHS;
	savedelta = BPNN_EPOCHS;

#if BPNN_TYPE
	char userid[40];
	std::string suserid;
	for (int i = 0; i < train_n; i++) {
		sscanf(NAME(trainlist->list[i]), "%[^_]_", userid);
		usernames.insert(suserid.assign(userid));
	}
	...
	if ((net = bpnn_read(netname)) == NULL) {
		if (train_n > 0) {
			...
			*/
			net = bpnn_create(imgsize, /*25: original number 4*/ BPNN_N_HIDDEN, BPNN_N_OUT);
		} else {
	...
	for (epoch = 1; epoch <= epochs; epoch++) {
		...
		for (i = 0; i < train_n; i++) {
			...
			bpnn_train(net, BPNN_ETA, BPNN_MOMENTUM, &out_err, &hid_err);
...
int evaluate_performance(BPNN *net,double *err)
{
	...
	//	printf("%.2LF-", net->target[i]);
	//printf("\n");
	//printf("output: ");
	//for (int i = 1; i <= BPNN_N_OUT; ++i)
	//	printf("%.2LF-", net->output_units[i]);
	//outcount++;
	//if (outcount > 20)
	//	exit(1);
	...
	*err = 0;
	for (int i = 1; i <= BPNN_N_OUT; ++i) {
		double delta = net->target[i] - net->output_units[i];
		*err += delta * delta;
	}
	*err *= 0.5;

	for (int i = 1; i <= BPNN_N_OUT; ++i) {
		if ((net->output_units[i] > 0.5 && net->target[i] > 0.5) ||
			(net->output_units[i] <= 0.5 && net->target[i] <= 0.5)) {
			continue;
		}
		else
		{
